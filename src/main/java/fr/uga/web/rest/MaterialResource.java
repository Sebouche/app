...
/**
 * REST controller for managing {@link fr.uga.domain.Material}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class MaterialResource {
    ...
    public List<Material> getAllMaterials(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Materials");
        return materialRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Material> getMaterial(@PathVariable Long id) {
        ...
        Optional<Material> material = materialRepository.findOneWithEagerRelationships(id);
